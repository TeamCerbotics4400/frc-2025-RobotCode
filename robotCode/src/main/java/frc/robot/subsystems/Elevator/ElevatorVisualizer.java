...
public class ElevatorVisualizer {
  ...
  public ElevatorVisualizer(String key, Color color) {
    ...
    LoggedMechanismRoot2d root = mechanism.getRoot("pivot", 1.68, 0.1);   //Posicion en la que se ubica
    elevator = new LoggedMechanismLigament2d("Elevator", 0.6, 90.0, 6, new Color8Bit(color));
    root.append(elevator);
  }

  /** Update elevator visualizer with current elevator angle */
  public void update(double position) {
    // Log Mechanism2d
    elevator.setLength(position);
    Logger.recordOutput("Elevator/Mechanism2d/" + key, mechanism);
    // Log 3D poses
    Pose3d pivot = new Pose3d(-0.0, 0.0, 0.0, new Rotation3d(0.0, 0, 0.0)); //Cambiar este visualmente
  ...
  public Pose3d getElevatorDistalPose(Pose3d armProximalPose, double position) {
    double elevatorExtentionLength = position;
    return armProximalPose.transformBy(
        new Transform3d(new Translation3d(0.0, 0.0, elevatorExtentionLength), new Rotation3d())); //Hace q se mueva
  }
}